...
export default function SubscriptionPlans() {
  ...
  return (
    <section className="space-y-6">
      ...
      {!loading && plans.length > 0 && !enabled ? <p role="status" className="text-sm text-muted">{sv ? 'Abonnemang öppnar snart. ' : 'Subscriptions open soon. '}<a href="/upload" className="text-pine underline">{sv ? 'Du kan redan nu skapa en gratis länk (minst 10 dollar, 5 procent).' : 'You can create a free link now ($10 minimum, 5%).'}</a></p> : null}
      {chosen && !current ? <div className="nl-card space-y-3 rounded-xl p-5">
