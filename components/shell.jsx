...
export function LangToggle() {
  ...
  return (
    <div className="flex rounded-sm bg-sheet/80 p-0.5" style={{ boxShadow: '0 0 0 1px rgba(255,255,255,.08)' }}>
      {['en', 'sv'].map((code) => (
        <button key={code} type="button" aria-pressed={locale === code} className={`min-h-11 min-w-11 rounded-xs px-2.5 font-mono text-[11px] font-medium uppercase tracking-wider ${locale === code ? 'bg-white/10 text-ink' : 'text-muted'}`} onClick={() => setLang(code)}>{code}</button>
      ))}
    </div>
  )
}
...
export function SiteFooter() {
  ...
  return (
    <footer className="relative z-10 mt-auto border-t border-line/80 bg-sheet/70 px-4 py-8 sm:px-6 lg:px-8" style={{ paddingBottom: 'max(2rem, env(safe-area-inset-bottom))' }}>
      <div className="mx-auto w-full max-w-7xl">
        ...
        <p className="mt-4 font-mono text-[10px] uppercase tracking-[0.22em] text-muted"><a href="https://www.nyttolabs.com" className="text-pine no-underline hover:opacity-80">www.nyttolabs.com</a></p>
      </div>
    </footer>
  )
}
