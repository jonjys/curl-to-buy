...
const sellerListingPrefix = 'seller-listings/'
...
function publicSeller(seller) {
  ...
  const { email, contactEmail, sellerContact, ...rest } = seller
  return rest
}

async function readJson(pathname) {
  try {
    const found = await readStoredBlob(pathname, get)
...
export async function saveListing(listing) {
  ...
  if (safe(listing.sellerId) && safe(listing.id)) {
    await writeJson(`${sellerListingPrefix}${listing.sellerId}/${listing.id}.json`, { id: listing.id })
  }
  return listing
}
...
export async function listSellerListingIds(sellerId, cursor) {
  if (!safe(sellerId)) return { ids: [], nextCursor: null, indexed: false }
  const page = await list({
    prefix: `${sellerListingPrefix}${sellerId}/`,
    limit: 50,
    ...(cursor ? { cursor } : {}),
  })
  const prefix = `${sellerListingPrefix}${sellerId}/`
  const ids = page.blobs
    .map((blob) => (blob.pathname.startsWith(prefix) ? blob.pathname.slice(prefix.length).replace(/\.json$/, '') : ''))
    .filter((id) => safe(id))
  return { ids, nextCursor: page.hasMore ? page.cursor : null, indexed: page.blobs.length > 0 || Boolean(cursor) }
}

export async function saveSeller(seller) {
  await writeJson(`${sellerPrefix}${seller.id}.json`, publicSeller(seller))
...
export async function updateSeller(id, changes) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    const found = await readStoredBlob(pathname, get)
    if (!found) return null
    const seller = JSON.parse(await new Response(found.result.stream).text())
    const next = publicSeller({ ...seller, ...changes, updatedAt: Date.now() })
    try {
      await put(pathname, JSON.stringify(next), {
        access: found.access,
        addRandomSuffix: false,
        ifMatch: found.result.blob.etag,
        contentType: 'application/json',
      })
      return next
    } catch (error) {
...
export async function saveSellerEmailIndex(emailHash, sellerId) {
  ...
  await put(path, JSON.stringify({ sellerId }), {
    ...PRIVATE,
    addRandomSuffix: false,
...
export async function consumeDownload(sessionId, fileIndex, limit) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    const found = await readStoredBlob(pathname, get)
    const current = found ? JSON.parse(await new Response(found.result.stream).text()) : null
    const count = Number(current?.count) || 0
    if (count >= limit) return { allowed: false, remaining: 0 }
    try {
      await put(pathname, JSON.stringify({ count: count + 1, updatedAt: Date.now() }), {
        access: found?.access || 'private',
        addRandomSuffix: false,
        contentType: 'application/json',
        ...(found ? { ifMatch: found.result.blob.etag } : { allowOverwrite: false }),
      })
...
export async function setListingPaused(id, sellerId, paused) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    if (!found) return null
    const listing = JSON.parse(await new Response(found.result.stream).text())
