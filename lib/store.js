...
function publicSeller(seller) {
  ...
  // Keep the seller email. Stripe setup needs it on later visits, and the
  // record is stored with private access.
  const { contactEmail, sellerContact, ...rest } = seller
  return rest
}

export async function assertPrivateBlobAccess() {
  try {
    // Put is what production rejects on a public store. Do this before creating
    // a Stripe account so a storage failure cannot leave an unused account.
    await put('sellers/storage-probe', '{}', {
