...
function planFromVerifiedId(key, price, spec) {
  if (!price || price.id !== spec.priceId) throw new Error(`Curl-to-Buy ${key} price is not configured correctly.`)
  if (price.currency !== 'eur' || price.recurring?.interval !== 'month' || price.recurring?.interval_count !== 1) {
    throw new Error(`Curl-to-Buy ${key} price is not configured correctly.`)
  }
  if (price.unit_amount !== spec.amount) throw new Error(`Curl-to-Buy ${key} price is not configured correctly.`)
  if (price.lookup_key && price.lookup_key !== spec.lookup) throw new Error(`Curl-to-Buy ${key} price is not configured correctly.`)
