...
function planFromVerifiedId(key, price, spec) {
  if (!price || price.id !== spec.priceId) throw new Error(`GetPaidLink ${key} price is not configured correctly.`)
  if (price.currency !== 'eur' || price.recurring?.interval !== 'month' || price.recurring?.interval_count !== 1) {
    throw new Error(`GetPaidLink ${key} price is not configured correctly.`)
  }
  if (price.unit_amount !== spec.amount) throw new Error(`GetPaidLink ${key} price is not configured correctly.`)
  if (price.lookup_key && price.lookup_key !== spec.lookup) throw new Error(`GetPaidLink ${key} price is not configured correctly.`)
  return { key, name: NAMES[key], priceId: price.id, currency: 'eur', amount: spec.amount, monthlyLinks: spec.monthlyLinks }
}
...
async function getPlansById(client) {
  ...
  for (const [key, spec] of Object.entries(VERIFIED_PLANS)) {
    ...
    let plan
    try {
      plan = planFromListedPrice(key, price)
    } catch {
      plan = planFromVerifiedId(key, price, spec)
    }
