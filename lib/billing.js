...
export function billingEnabled() {
  ...
  // Preview must never start live subscriptions. Test storage is a separate Blob store.
  if (process.env.VERCEL_ENV === 'preview' && !/^(sk|rk)_test_/.test(key)) return false
  return Boolean(key && process.env.STRIPE_CTB_BILLING_ENABLED === 'true'
    && process.env.STRIPE_CTB_WEBHOOK_SECRET
...
function planFromListedPrice(key, price) {
  ...
  if (!price || price.active !== true || !product || typeof product === 'string' || product.active !== true
    ...
    throw new Error(`GetPaidLink ${key} price is not configured correctly.`)
  }
  const monthlyLinks = safeQuota(product.metadata?.monthly_links)
  if (monthlyLinks === undefined) throw new Error(`GetPaidLink ${key} link limit is invalid.`)
  return { key, name: NAMES[key], priceId: price.id, currency: price.currency, amount: price.unit_amount, monthlyLinks }
}

function assertPriceMode(price) {
  const key = process.env.STRIPE_SECRET_KEY || ''
  const live = /^(sk|rk)_live_/.test(key)
  if (price.livemode !== live) throw Error('Stripe price mode does not match the deployment.')
}
...
async function getPlansById(client) {
  ...
  for (const [key, spec] of Object.entries(VERIFIED_PLANS)) {
    ...
    if (price.id !== spec.priceId || plan.amount !== spec.amount || plan.monthlyLinks !== spec.monthlyLinks) {
      throw new Error(`GetPaidLink ${key} price is not configured correctly.`)
    }
    assertPriceMode(price)
    plans.push(plan)
  }
  return plans
}
...
async function getPlansByLookup(client) {
  ...
  return Object.keys(VERIFIED_PLANS).map((key) => {
    ...
    const plan = planFromListedPrice(key, price)
    const spec = VERIFIED_PLANS[key]
    if (plan.amount !== spec.amount || plan.monthlyLinks !== spec.monthlyLinks) throw Error('Unexpected plan terms.')
    assertPriceMode(price)
    return plan
  })
}
...
export function portalConfiguration() {
  ...
  return /^bpc_[A-Za-z0-9]+$/.test(id) ? id : null
}
