...
export async function publishListing(seller, body, listing) {
  ...
  if (!billing.active) {
    throw Object.assign(Error(locale === 'sv'
      ? 'Välj ett abonnemang för att publicera länken.'
      : 'Choose a subscription to publish your link.'), { status: 402, needsPlan: true })
  }
  await reserveLink(seller.id, { ...billing, locale }, id)
