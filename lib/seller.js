...
// Recovery: a seller who loses their cookie (new phone, cleared cookies,
// different browser) proves ownership via a one-time link emailed to a
// recovery address they registered while they still held a valid cookie.
// The email itself is not a secret — it's stored plainly on the seller
// record so we can send to it — but is looked up only through its hash, so
// listing the raw store never exposes which address maps to which seller.
export function newRecoveryToken() {
  return randomBytes(32).toString('hex')
}
