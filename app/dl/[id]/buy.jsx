...
export default function BuyBox({ listing, price }) {
  ...
  return (
    <div className="nl-card nl-card-glow rounded-2xl p-6 sm:p-8">
      ...
      {isPhysical ? <p className="mt-2 text-xs leading-relaxed text-muted">{sv ? 'Säljaren ansvarar för att skicka varan. GetPaidLink erbjuder inte köparskydd eller egen frakt.' : 'The seller is responsible for shipping. GetPaidLink does not provide buyer protection or shipping.'}</p> : null}
      ...
      <button type="button" onClick={pay} disabled={busy || listing.soldOut || expired} className="mt-8 inline-flex h-12 w-full items-center justify-center rounded-xl bg-pine px-5 text-base font-bold text-pine-fg disabled:opacity-50">{listing.soldOut ? (isPhysical ? (sv ? 'Såld' : 'Sold') : t.soldOut) : expired ? t.offerExpired : busy ? t.verifying : `${isPhysical ? (sv ? 'Betala' : 'Pay') : t.pay} ${price.label}`}</button>
