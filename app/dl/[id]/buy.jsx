...
export default function BuyBox({ listing, price }) {
  ...
  return (
    <div className="nl-card nl-card-glow rounded-2xl p-6 sm:p-8">
      <p className="text-xs font-bold uppercase tracking-kicker text-pine">{t.buyKicker}</p>
      ...
      <p className="mt-1 text-sm text-muted">{listing.fileCount} {listing.fileCount === 1 ? t.oneFile : t.manyFiles}</p>
      ...
      {remaining != null || listing.expiresAt ? (
        <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-1">
          {remaining != null ? (
            <p className="text-sm font-bold text-warn">{remaining} {t.left}</p>
          ) : null}
          {listing.expiresAt && !expired ? (
            <p className="font-mono text-sm font-bold tabular-nums text-warn">{t.timeLeft} {formatCountdown(msLeft)}</p>
          ) : null}
        </div>
      ) : null}
      ...
      <button
        type="button"
        onClick={pay}
        disabled={busy || listing.soldOut || expired}
        className="mt-8 inline-flex h-12 w-full items-center justify-center rounded-xl bg-pine px-5 text-base font-bold text-pine-fg disabled:opacity-50"
      >
        {listing.soldOut ? t.soldOut : expired ? t.offerExpired : busy ? t.verifying : `${t.pay} ${price.label}`}
      </button>
